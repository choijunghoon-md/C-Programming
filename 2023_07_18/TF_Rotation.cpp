...
void TF_base_link(Point2D* base_link_map_point2d, Pose2D base_link_origin) {
	set_rotation_matrix_inverse(base_link_origin.theta);
	double transform_X = (Rotation_matrix_inverse[0][0] * (base_link_map_point2d->x - base_link_origin.x)) + (Rotation_matrix_inverse[0][1] * (base_link_map_point2d->y - base_link_origin.y));
	double transform_Y = (Rotation_matrix_inverse[1][0] * (base_link_map_point2d->x - base_link_origin.x)) + (Rotation_matrix_inverse[1][1] * (base_link_map_point2d->y - base_link_origin.y));

	//테스트용 출력
	printf("TF base link         %6.3f  %6.3f\n", transform_X, transform_Y);
}

void TF_base_link_map(Point2D base_link_point2d, Pose2D base_link_origin) {
	set_rotation_matrix(base_link_origin.theta);
	double transform_X = (Rotation_matrix[0][0] * base_link_point2d.x) + (Rotation_matrix[0][1] * base_link_point2d.y) + base_link_origin.x;
	double transform_Y = (Rotation_matrix[1][0] * base_link_point2d.x) + (Rotation_matrix[1][1] * base_link_point2d.y) + base_link_origin.y;

	//테스트용 출력
	printf("TF base link map     %6.3f  %6.3f\n", transform_X, transform_Y);
}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	base_link_Point2D.y = 0.0;

	base_link_map_Point2D.x = 0.0;
	...
	TF_base_link(&base_link_map_Point2D, base_link_origin);
	TF_base_link_map(base_link_Point2D, base_link_origin);
